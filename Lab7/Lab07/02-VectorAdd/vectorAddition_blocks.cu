/*******************************************************************************
*                                                                              *
*  vectorAddition_blocks.cu -                                                  *
*                                                                              *
*   Adds two vectors on the device (GPU) using several blocks; one thread per  *
*   block
*                                                                              *
...
{
  result [blockIdx.x] = augend [blockIdx.x] + addend [blockIdx.x];

  printf ( "On block %d, thread %d: %d + %d = %d\n", blockIdx.x, threadIdx.x, \
            augend [blockIdx.x], addend [blockIdx.x], result [blockIdx.x] );
}
...
int main ( void )
{
  ...
  add <<< VEC_LEN, 1 >>> ( d_augend, d_addend, d_result );
