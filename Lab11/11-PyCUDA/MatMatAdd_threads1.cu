/*******************************************************************************
*
*  MatMatAdd_threads1.cu -  A program to add two matrices using CUDA
*
...
__global__ void MatAddKernel ( int * MA, int * MB, int * MY, int size )
{
  ...
  {
    for ( int i = 0; i < N; i++ )
      MY[row * size + col] = MA[row * size + col] + MB[row * size + col];
  }
}
...
int main ( void )
{
  ...
  MatAddKernel <<< BlocksPerGrid, ThreadsPerBlock >>> ( d_MA, d_MB, d_MY, N );
